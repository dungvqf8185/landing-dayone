...
// Compatibility shim: restore Model.drawables.renderOrders for pixi-live2d-display 0.4.x.
//
// In Core 06.x (Cubism 5):
//  1) renderOrders moved from Drawables to Model.
//  2) Model.renderOrders has length (drawableCount + offscreenCount), not just
//     drawableCount. Models using new offscreen feature (Ren has PartDisplay/
//     PartHologram) get renderOrder values >= drawableCount for some drawables.
//
// pixi-live2d-display 0.4.0's render loop:
//   for(i=0; i<drawableCount; i++) sortedList[renderOrder[i]] = i;
//   for(i=0; i<drawableCount; i++) drawMesh(sortedList[i]);
//
// If renderOrder[i] >= drawableCount, the drawable is written to a slot that's
// never read → drawable is silently dropped. For Ren this hides mouth/eye
// drawables which sit on top of the model (highest render orders).
//
// Fix: expose drawables.renderOrders as a *getter* that recomputes ranks each
// call by sorting Model.renderOrders[0..drawableCount) into contiguous indices
// [0, drawableCount). This preserves Z-order while ensuring every slot in the
// sorted list is filled.
;(function(){
  if(typeof Live2DCubismCore==="undefined"||!Live2DCubismCore.Model) return;
  var M=Live2DCubismCore.Model;
  var orig=M.fromMoc;
  M.fromMoc=function(moc){
    var model=orig.call(M,moc);
    if(model&&model.drawables&&model.renderOrders){
      var drawableCount=model.drawables.count;
      var srcOrders=model.renderOrders;
      var cache=new Int32Array(drawableCount);
      var idxArr=new Int32Array(drawableCount);
      var cmp=function(a,b){return srcOrders[a]-srcOrders[b];};
      // Replace any pre-existing assignment from older shim with a getter.
      try{delete model.drawables.renderOrders;}catch(e){}
      Object.defineProperty(model.drawables,"renderOrders",{
        configurable:true,
        get:function(){
          // Reset idxArr to identity permutation [0..N-1] each call so sort
          // operates on a fresh ordering. Sort by srcOrders[idx] ascending,
          // then assign rank r to the original drawable index idxArr[r].
          for(var k=0;k<drawableCount;k++){idxArr[k]=k;}
          // Int32Array.sort doesn't take comparator in older engines reliably;
          // use Array.from for cross-version safety, then write back.
          var arr=Array.prototype.slice.call(idxArr);
          arr.sort(cmp);
          for(var r=0;r<drawableCount;r++){cache[arr[r]]=r;}
          return cache;
        }
      });
    }
